...
st.set_page_config(page_title="AIROS - Dashboard", page_icon="🏠", layout="wide")
...
with col1:
    st.metric(label="Total Applications", value=metrics["applications"])

with col2:
    st.metric(label="Interviews / Screenings", value=metrics["interviews"])

with col3:
    st.metric(label="Avg ATS Match Score", value=f"{metrics['avg_ats_score']}%")
